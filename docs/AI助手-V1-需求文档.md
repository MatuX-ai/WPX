...
**版本**：V1.1（业务模式变更为「完全免费」）  
**状态**：已实现（MVP）· **当前为完全免费模式**  
...
- [ ] **用户在「我的模型」中配置任意大模型 API 后，可与该模型正常对话**（V1.1 不提供任何平台模型）
- [ ] 未配置任何模型时，打开对话窗显示引导卡片与「去配置」按钮
- [ ] 对话窗顶部可下拉切换已配置的多个模型
- [ ] 编辑器选中文本后，底部出现选区提示  
- [ ] 输入框激活时显示选中文本预览  
...
- [ ] 无选区时对话不修改文档内容  
- [ ] 对话窗不遮挡右下角头像  
- [ ] API Key 仅本地 AES 加密存储，不出现在日志、调试面板、导出文件等任何明文位置  
