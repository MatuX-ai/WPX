...
与 [WPX 用户注册与账户系统需求文档.md](../../WPX%20用户注册与账户系统需求文档.md) 对齐。
...
> - **V1.1 起不再提供免费大模型 / Token 额度**：用户须自行在「我的模型」配置 API Key
...
**前置**：清除凭据（删除用户数据目录中的 auth store，或全新安装）。
...
1. 打开 AI 写作助手，发送消息
2. 提示含：**尚未接入大模型，我无法帮你写作**
3. 有 **「设置」** / **「自己写」** 按钮；设置可跳转「我的模型」，自己写关闭 AI 面板
...
1. **我的模型** 仅支持用户自定义模型，无「WPX 公共大模型」选项
2. 测试连接：成功绿色 ✓，失败红色 ✕
...
**前置**：确保当前账户未在「我的模型」中配置任何自定义 API。

1. 打开 AI 写作助手，发送需要 AI 思考的消息（非本地指令）
2. 提示：**尚未接入大模型，我无法帮你写作**
3. 点击 **设置** 跳转「我的模型」；或点 **自己写** 关闭面板自行写作
4. 配置自定义 Key 后重新发送，应正常调用用户自有模型
