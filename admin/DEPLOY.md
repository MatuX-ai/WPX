...
本文档说明如何将 `admin/` 项目构建并部署到 `admin.proclaw.cc`，包含 **Cloudflare Pages** 与 **Vercel** 两种方案（与 landing 项目共享同服务商）。
...
[浏览器] https://admin.proclaw.cc
   ...
   └── /api/* 业务请求 ───────┘         │
                              ...
                              Functions / Serverless 反向代理
                                         │
                                         ▼
                              https://api.proclaw.cc/admin/*
                              https://account.proclaw.cc (CORS)
```

- **业务 API**：`/api/*` 通过 Pages Functions / Vercel Serverless Function 反向代理到 `api.proclaw.cc/admin`，避免 CORS
- **认证 API**：登录页直接调用 `account.proclaw.cc`，由 account 服务自身开启 CORS 放行 `https://admin.proclaw.cc`
...
- `admin/functions/api/[[path]].js` 处理 `/api/*` 全部请求
- `admin/public/_redirects` 配置 SPA fallback：`/* → /index.html 200`
...
2. 输入 `admin.proclaw.cc`
3. Cloudflare 会要求在 `proclaw.cc` 域名 DNS 添加 CNAME 记录：
   - **Name**: `admin`
   - **Target**: `wpx-admin.pages.dev`（首次配置时显示）
   - **Proxy**: ✓ Proxied（橙色云朵）
...
- **Cloudflare Pages**：`functions/api/[[path]].js` 转发 `/api/*` → `https://api.proclaw.cc/admin/*`
- **Vercel**：`api/proxy.js` 转发 `/api/*` → `https://api.proclaw.cc/admin/*`
...
```bash
# Cloudflare Pages
API_TARGET=https://staging-api.proclaw.cc/admin
...
```env
VITE_API_BASE_URL=https://api.proclaw.cc/admin
```

此时需要后端在响应头中放行 `https://admin.proclaw.cc`：

```nginx
# Nginx 示例
add_header Access-Control-Allow-Origin https://admin.proclaw.cc always;
...
`account.proclaw.cc` 是独立的 JWT 认证服务，必须在它的 CORS 白名单中添加：

```
Access-Control-Allow-Origin: https://admin.proclaw.cc
...
| `VITE_API_BASE_URL` | ✓ | `/api` | 业务 API 入口。推荐走反代 |
| `VITE_ACCOUNT_BASE_URL` | ✓ | `https://account.proclaw.cc` | 认证服务 |
| `API_TARGET` | ✗ | `https://api.proclaw.cc/admin` | 反代目标地址（仅 Cloudflare Pages / Vercel Functions 使用） |
...
| `VITE_ACCOUNT_BASE_URL` | `https://account.proclaw.cc` | Production / Preview |
| `API_TARGET` | `https://api.proclaw.cc/admin` | Production |
| `API_TARGET` | `https://staging-api.proclaw.cc/admin` | Preview |
...
| `VITE_ACCOUNT_BASE_URL` | `https://account.proclaw.cc` | All |
| `API_TARGET` | `https://api.proclaw.cc/admin` | Production |
...
1. 在后端数据库插入初始超级管理员（邮箱 `super@proclaw.cc`，初始密码 `Wpx@2026!Init`）
2. 访问 https://admin.proclaw.cc/login
3. 使用上述凭据登录
4. **立即修改初始密码**
5. 创建团队成员账号
...
| ✓ | 项目 | 验证方式 |
|---|------|----------|
| ☐ | `https://admin.proclaw.cc` 可访问且 HTTPS 锁图标正常 | 浏览器打开 |
| ☐ | 首页 200，控制台无 404 | DevTools Network |
| ☐ | 登录页正常渲染 | `/login` |
| ☐ | 凭据登录成功，token 写入 localStorage | DevTools Application |
| ☐ | 登录后跳转 `/dashboard` 且数据正常 | 仪表盘 6 个指标 |
| ☐ | `/api/*` 请求经 Functions/Serverless 反代 | Network → /api/orders/recharge 状态 200 |
| ☐ | 静态资源加载 200（`/assets/*.js`） | Network |
| ☐ | 浏览器刷新任意子页面（如 `/users`）不报 404 | F5 |
| ☐ | 401 拦截跳转到登录页 | 删除 token 后访问 `/dashboard` |
...
JWT 未被后端识别。检查 `account.proclaw.cc` 是否正确签发 token，以及 `http.js` 拦截器是否正确读取 `wpx_admin_token` localStorage 项。
...
如使用直连模式且遇到 CORS 错误，请改用反代模式（`VITE_API_BASE_URL=/api`），或在 `account.proclaw.cc` / `api.proclaw.cc` 响应头中添加 `Access-Control-Allow-Origin: https://admin.proclaw.cc`。

### Q6. 自定义构建参数

如需自定义 base path（如 `/admin/`），修改 `vite.config.js`：
...
并相应调整 `vercel.json` 的 `rewrites.source` 和 `public/_redirects` 的 `/* /index.html 200` 为 `/* /admin/index.html 200`。
